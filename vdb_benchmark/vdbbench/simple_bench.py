"""
simple_bench.py - Vector Database Benchmark with Recall Metrics
...
Recall metric design (addresses review comments on PR #7):
  1. Ground truth is pre-computed OUTSIDE the timed benchmark using a
     duplicate FLAT-indexed collection (brute-force exact search).
     This avoids doubling queries (which halved throughput in the original
     approach) and avoids circular ANN-validating-ANN logic.
  2. batch_end timing is placed BEFORE any recall-related result capture,
     so performance numbers (QPS, latency) only reflect the primary search.
  3. Query vectors are pre-generated with a fixed seed before the benchmark
     to ensure identical queries hit both FLAT (ground truth) and ANN
     (benchmark) collections.

Recall calculation follows the VectorDBBench methodology:
  recall@k = |ANN_top_k ∩ FLAT_top_k| / k
"""
